    ...
    public class ModeloLiberacion
    {
        ...
        public DataTable ObtenerDatosLiberación(string NroFianza, int año) // Botones "Consulta" y "Liberación"
        {
            ...
            try
            {
                ...
                if (año >= 0 && año <= 2005)
                {
                    cmd.CommandText = "SELECT [Fecha Liberacion], Cod_Lib FROM [Copia de Fianzas] WHERE [Nro de Fianza] = '" + NroFianza + "' AND Ano = " + año;
                    cmd.CommandType = CommandType.Text;
                    OleDbDataReader reader = cmd.ExecuteReader();
                ...
                if (año == 2004 || año == 2005)
                    if (dt.Rows.Count == 0)
                    {
                        cmd.CommandText = "SELECT [Fecha Liberacion], Cod_Lib FROM Fianzas WHERE [Nro de Fianza] = '" + NroFianza + "' AND Ano = " + año;
                        cmd.CommandType = CommandType.Text;
                        OleDbDataReader reader = cmd.ExecuteReader();
                        dt = CargarTabla(reader, dt);
                        reader.Close();
