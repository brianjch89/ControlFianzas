        ...
        public int GestionarFianza(DataTable dt, int opc, int año)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        cmd.CommandText = "UPDATE [Copia de Fianzas] SET [Fecha de Emision] = '" + Convert.ToDateTime(f["Fecha de Emision"].ToString()).Date + "', [Nro Archivo] = " + long.Parse(f["Nro Archivo"].ToString()) + ", [Fecha de Vencimiento] = '" + Convert.ToDateTime(f["Fecha de Vencimiento"].ToString()).Date + "', [Fecha Recibida] = '" + Convert.ToDateTime(f["Fecha Recibida"].ToString()).Date + "', Garante = '" + f["Nombre_Garante"].ToString() + "' , [Rif Contribuyente] = '" + f["Nombre_Consignatario"].ToString() + "', [Rif Agente] = '" + f["Nombre_Agente"].ToString() + "'," +
                                          ...
                                          "WHERE [Nro de Fianza] = '" + f["Nro de Fianza"].ToString() + "' AND Ano = " + Convert.ToInt32(f["Ano"].ToString());
                        cmd.CommandType = CommandType.Text;
                    ...
                    if (año == 2004 || año == 2005)
                        if (filasAfectadas == 0)
                        {
                            cmd.CommandText = "UPDATE Fianzas SET [Fecha de Emision] = '" + Convert.ToDateTime(f["Fecha de Emision"].ToString()).Date + "', [Nro Archivo] = " + long.Parse(f["Nro Archivo"].ToString()) + ", [Fecha de Vencimiento] = '" + Convert.ToDateTime(f["Fecha de Vencimiento"].ToString()).Date + "', [Fecha Recibida] = '" + Convert.ToDateTime(f["Fecha Recibida"].ToString()).Date + "', Garante = '" + f["Nombre_Garante"].ToString() + "' , [Rif Contribuyente] = '" + f["Nombre_Consignatario"].ToString() + "', [Rif Agente] = '" + f["Nombre_Agente"].ToString() + "'," +
                                              "Cod_Tf = '" + Convert.ToByte(f["NaturalezaOperacion"].ToString()) + "', Asunto = '" + f["Asunto"].ToString() + "', Monto = " + f["Monto"].ToString() + ", Notaria = '" + f["Notaria"].ToString() + "', Tomo = '" + f["Tomo"].ToString() + "', Nro = '" + f["Nro"].ToString() + "', [Fecha de Reg] = '" + Convert.ToDateTime(f["Fecha de Reg"].ToString()).Date + "', NroOficio = " + long.Parse(f["NroOficio"].ToString()) + " " +
                                              "WHERE [Nro de Fianza] = '" + f["Nro de Fianza"].ToString() + "' AND Ano = " + Convert.ToInt32(f["Ano"].ToString());
                            cmd.CommandType = CommandType.Text;
                            n = cmd.ExecuteNonQuery();
